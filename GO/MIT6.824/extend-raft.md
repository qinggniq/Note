...
1. 如果`term < currentTerm`返回 `false` （**接受者行为1**）
2. 如果 `votedFor` 为空或者为 `candidateId`，并且候选人的日志至少和自己一样新，那么就投票给他 （**接受者行为2**）
...
2. 因为超时而导致选举失败，如果在选举超时时间中**没有任何一个Candidate获得绝大多数节点的支持**，那么此次选举失败，立刻开启下一轮选举；
3. 因为其他Leader选举成功而选举失败，也就是在选举过程中**有一个Candidate获得了绝大多数节点的支持**，那么新选举出的Leader会发送心跳包给自己
